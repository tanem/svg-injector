...
- IRI renumeration no longer rewrites references to ids that only `Object.prototype` defines. A reference such as `url(#constructor)` or `href="#valueOf"` is left alone now, the same as any other reference to an id the file does not define.
